...
// per-keystroke cost to a single search. Errors are cached as the same null the
// caller sees, so an invalid prefix is not re-run over the whole document on
// every keystroke. The cache lives inside the closure over `data`, so it never
// outlives the document it was built for.
export function createScopeResolver(
  data: JsonValue
): (leftExpr: string) => JsonValue | null {
  let cache: { expr: string; value: JsonValue | null } | null = null;
  return (leftExpr: string): JsonValue | null => {
    ...
    const value = outcome.ok ? outcome.result : null;
    cache = { expr: leftExpr, value };
    return value;
  };
}
