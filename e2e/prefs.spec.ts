...
test("tree view and explicit level restored after reload", async () => {
  ...
  const levelBtns = await page.$$("#jv-levels button");
  await levelBtns[1].click(); // "2"
  await page.waitForTimeout(600);
  await load("127.0.0.1");
  expect(await activeView()).toBe("tree");
  await expect(page.locator("#jv-levels button.jv-active")).toHaveText("2");
});
...
test("a different origin keeps defaults", async () => {
  ...
  expect(await activeView()).toBe("tree");
  await expect(page.locator("#jv-levels button.jv-active")).toHaveText("All");
});
